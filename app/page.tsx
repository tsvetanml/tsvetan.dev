  ...
  return (
    <div className="min-h-screen flex items-center justify-center py-12">
      <div className="max-w-lg sm:max-w-xl md:sm:max-w-3xl lg:sm:max-w-4xl w-full space-y-8 text-center">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center my-8 sm:space-x-4 space-y-4 sm:space-y-0 w-full mt-8 sm:mt-4">
          {actions.map((action: any, index: number) => {
            if (action.type === NavigationItemType.LINK) {
              return (
                <>
                  <div className="">
                    <Link className="inline-flex items-center bg-blue-600 bg-opacity-80 text-white font-semibold justify-center w-full sm:w-auto backdrop-filter backdrop-blur-sm rounded-lg cursor-pointer px-8 py-2 border-2 border-blue-900 border-opacity-60" href={action.href}>
                      {action.icon}
                      {action.text}
                    </Link>
                  </div>
